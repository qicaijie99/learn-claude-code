...
def assemble_system_prompt(context: dict) -> str:
    ...
    active = context.get("active_teammates", [])
    if active:
        sections.append(f"Active teammates: {', '.join(active)}")
    ...
    sections.append(f"Current time: {context.get('current_time', 'unknown')}")
...
def update_context(context: dict, messages: list) -> dict:
    ...
    return {
        "enabled_tools": list(tool["name"] for tool in tools),
        "workspace": str(WORKDIR),
        "skills": list_skills(),
        "memory_index": memory_index,
        "memories": relevant_memories,
        "mcp_servers": sorted(mcp_clients.keys()),
        "active_teammates": sorted(active_teammates.keys()),
...
_MEMORY_SLUG_RE = re.compile(r"[^a-z0-9._-]+")


def memory_path(name: str) -> Path:
    normalized = name.strip().lower()
    normalized = normalized.replace("\\", "-").replace("/", "-")
    slug = _MEMORY_SLUG_RE.sub("-", normalized).strip("._-")

    if not slug:
        raise ValueError("Memory name produces an empty filename")

    path = (MEMORY_DIR / f"{slug}.md").resolve()
    root = MEMORY_DIR.resolve()

    if not path.is_relative_to(root):
        raise ValueError(f"Memory path escapes memory directory: {name}")

    return path

def write_memory_file(name: str, mem_type: str, description: str, body: str, rebuild_index: bool = True,):
...
SUB_TOOLS = [
    ...
    {"name": "read_file", "description": "Read file contents.",
     "input_schema": {"type": "object", "properties": {"file_path": {"type": "string"},"limit": {"type": "integer", "minimum": 0},
    "offset": {"type": "integer", "minimum": 0},}, "required": ["file_path"]}},
...
def run_read(file_path: str, limit: int | None = None, offset: int = 0, cwd: Path | None = None) -> str:  # 可限制返回line个数
    try:
        lines = safe_path(file_path, cwd).read_text(encoding="utf-8", errors="replace").splitlines() # 把读到的字符串按行分割成list = ["line1", "line2", ...]
        offset = max(int(offset or 0), 0)
        remaining = lines[offset:]
        if limit is not None:
            limit = max(int(limit), 0)
            visible = remaining[:limit]
            omitted = len(remaining) - len(visible)
            if omitted:
...
def run_write(file_path: str, content: str, cwd: Path | None = None) -> str:
    try:
        file_path = safe_path(file_path, cwd)
...
def has_tool_use(content) -> bool:
    """Return True when the response actually contains a tool_use block."""
    if not isinstance(content, list):
        return False
    return any(get_block_type(block) == "tool_use" for block in content)
    ...
    for _ in range(attempts):
        ...
        for task in scan_unclaimed_tasks():
            ...
            if result.startswith("Claimed "):
                wt_info = ""
                if task.worktree:
                    wt_path = (WORKTREES_DIR / task.worktree).resolve()  #  将路径解析为规范的绝对路径
                    wt_info = f"\nWork directory: {wt_path}"
                    if worktree_context is not None:
                        worktree_context["path"] = str(wt_path)
                elif worktree_context is not None:
                ...
                messages.append({
                    ...
                    "content": (
                        ...
                        f"{wt_info}{task.description}</auto-claimed>"
                    ),
                })
...
def spawn_teammate_thread(name: str, role: str, prompt: str) -> str:
    ...
    def handle_inbox_message(name: str, msg: dict, messages: list) -> bool:
        ...
        if msg_type == "plan_approval_response":
            waiting_id = protocol_ctx["waiting_plan"]

            if not waiting_id or req_id != waiting_id:
                log_lifecycle(
                    name,
                    "unexpected_plan_response",
                    request_id=req_id,
            ...
            protocol_ctx["waiting_plan"] = None
            approve = bool(meta.get("approve", False))
            if approve:
    ...
    def run():
        ...
        def _run_read(file_path: str, limit: int | None = None, offset: int = 0) -> str:
            return run_read(file_path, limit=limit, offset=offset, cwd=_wt_cwd())

        def _run_write(file_path: str, content: str) -> str:
            return run_write(file_path, content, cwd=_wt_cwd())
        ...
        def _run_submit_plan(plan: str) -> str:
            output = _teammate_submit_plan(name, plan)
            match = re.search(r"\((req_\d+)\)", output)

            if not match:
                return f"Error: could not parse plan request id from: {output}"
            ...
            while True:
                ...
                for turn in range(1, MAX_WORK_TURNS + 1):
                    ...
                    for block in response.content:
                        ...
                        if gate_closed:
                            output = "Deferred: waiting for plan approval."
                        else:
...
def run_git(args: list[str]) -> tuple[bool, str]:
    ...
    try:
        r = subprocess.run(["git"] + args, cwd=WORKDIR,  #  subprocess来自标准库执行外部指令
...
def assemble_tool_pool() -> tuple[list[dict], dict]:
    ...
    for server_name, mcp_client in mcp_clients.items():
        ...
        for tool_def in mcp_client.tools:
            ...
            handlers[prefixed] = (
                lambda *, c=mcp_client, t=tool_def["name"], **kw: c.call_tool(t, kw))  #  单独的 * 表示：后面的参数必须用关键字传入，不能用位置参数传入。
...
BUILTIN_TOOLS = [
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {"file_path": {"type": "string"},"limit": {"type": "integer", "minimum": 0}, "offset": {"type": "integer", "minimum": 0},},
            "required": ["file_path"],
        },
    },
    {
